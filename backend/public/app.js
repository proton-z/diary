...
function setEdSelectedTags(tags) {
  const arr = Array.isArray(tags) ? tags : [];
  state.edSelectedTags =
...
function renderEdTagChips() {
  ...
  const tags = state.tags;
  el.innerHTML = tags.map((t) => {
                       const active = state.edSelectedTags.includes(t);
                       return `
        <button
          type="button"
          data-ed-tag="${esc(t)}"
  ...
  el.querySelectorAll('button[data-ed-tag]').forEach((btn) => {
    btn.addEventListener('click', () => {
      ...
      const has = state.edSelectedTags.includes(t);
      const next = has ? state.edSelectedTags.filter((x) => x !== t) :
                         [...state.edSelectedTags, t];
      setEdSelectedTags(next);
    });
  });
}
...
function tagPills(tags) {
  ...
  return arr.map((t) => `<span class="pill">${esc(t)}</span>`).join('');
}
...
function dueMeta(due_date, completed) {
  ...
  if (!d) return '';
  if (completed)
    return `<span class="text-xs text-slate-500">DDL：${esc(d)}</span>`;
  ...
  if (Number.isNaN(diffDays))
    return `<span class="text-xs text-slate-500">DDL：${esc(d)}</span>`;
  if (diffDays < 0)
    return `<span class="text-xs font-medium text-rose-200">逾期：${
        esc(d)}</span>`;
  if (diffDays <= 2)
    return `<span class="text-xs font-medium text-amber-200">临期（${
        diffDays} 天）：${esc(d)}</span>`;
  return `<span class="text-xs text-slate-400">DDL：${esc(d)}（${
      diffDays} 天）</span>`;
}
...
function rangeMeta(start_date, end_date) {
  ...
  if (!a && !b) return '';
  if (a && b)
    return `<span class="text-xs text-slate-400">持续：${esc(a)} ~ ${
        esc(b)}</span>`;
...
function renderTasks() {
  ...
  $('#viewMeta').textContent =
      state.tasks.length ? `共 ${state.tasks.length} 条` : '';
  const list = $('#taskList');
  ...
  list.innerHTML =
      state.tasks
          .map((t) => {
            ...
            return `
        <div class="task-row group flex items-start gap-3 px-4 py-3">
          <button
            class="${
                done ?
                    'bg-emerald-500/20 border-emerald-500/30' :
                    'bg-white border-black/10 hover:bg-black/5'} mt-0.5 h-5 w-5 rounded-md border"
            data-action="toggle"
            data-id="${t.id}"
            title="切换完成"
          ></button>
          ...
          <div class="min-w-0 flex-1">
            <div class="flex flex-wrap items-center gap-2">
              <div class="${
                done ? 'line-through text-slate-500' : ''} text-sm font-medium">
                ${esc(t.title)}
              </div>
              ${tagPills(t.tags || (t.tag ? [t.tag] : []))}
            </div>
            ${
                t.goal ?
                    `<div class="mt-1 text-xs leading-5 text-slate-700"><span class="text-slate-500">目标：</span>${
                        esc(t.goal)}</div>` :
                    ''}
            <div class="mt-2 flex flex-wrap items-center gap-2 text-xs log-meta">
...
function openEditor(task) {
  ...
  $('#edNewTag').value = '';
  const seedTags = Array.isArray(task?.tags) ?
      task.tags.filter((x) => typeof x === 'string' && x.trim())
          .map((x) => x.trim()) :
      (task?.tag ? [String(task.tag).trim()] : []);
  seedTags.forEach((tg) => ensureTagInList(tg));
  state.edSelectedTags = seedTags;
  renderEdTagChips();
...
async function saveEditor() {
  ...
  if (pendingNewTag) {
    ...
    state.edSelectedTags =
        Array.from(new Set([...(state.edSelectedTags || []), pendingNewTag]));
    $('#edNewTag').value = '';
  }
  const tags = Array.from(new Set((state.edSelectedTags || [])
  ...
  if (state.editingTask) {
    ...
    await api('/api/tasks', {
      method: 'POST',
      body: JSON.stringify({title, goal, tags, due_date, start_date, end_date})
    });
  }
...
function wire() {
  ...
  $('#btnEdAddTag')?.addEventListener('click', () => {
    ...
    $('#edNewTag').value = '';
    setEdSelectedTags(
        Array.from(new Set([...(state.edSelectedTags || []), v])));
  });
  ...
  $('#btnEdClearTag')?.addEventListener('click', () => setEdSelectedTags([]));
